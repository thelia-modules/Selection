...
class SelectionFolderImageUploadController extends FileController
{
    ...
    {
        $data = ['SELECTION_FOLDER' => "admin.Selection"];
        $module = 'Selection';

        $this->getAdminResources()->addModuleResources($data, $module);
        $this->getFileManager()->addFileModel('image', $parentType, SelectionFolderImage::class);
        ...
        return $this->render('selection-folder-image-upload-list-ajax', $args);
    }
    ...
    public function deleteFileAction($fileId, $parentType, $objectType, $eventName)
    {
        ...
        $this->getAdminResources()->addModuleResources($data, $module);
        $this->getFileManager()->addFileModel('document', $parentType, SelectionFolderImage::class);
        $this->checkAuth(
            $this->getAdminResources()
                ->getResource('selection_folder', static::MODULE_RIGHT),
            array(),
            AccessManager::UPDATE
        );
        $this->checkXmlHttpRequest();
    ...
    public function saveFileAjaxAction(
        ...
        $this->getAdminResources()->addModuleResources($data, $module);
        $this->getFileManager()->addFileModel($objectType, $parentType, SelectionFolderImage::class);
    ...
    public function viewImageAction($imageId, $parentType)
    {
        ...
        $image = SelectionFolderImageQuery::create()->findPk($imageId);

        $redirectUrl = 'admin/selection/update/'. $image->getSelectionFolderId();

        return $this->render('selection-folder-image-edit', array(
            'imageId' => $imageId,
